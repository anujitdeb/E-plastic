...
@section('subcontent')
    ...
    $show = 'student.show';
    $bulk_delete = 'student.bulk_delete';
    $bulk_active = 'student.bulk_active';
    $bulk_inactive = 'student.bulk_inactive';
    @endphp

    <h2 class="intro-y text-lg font-medium mt-10">Admin List </h2>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            @if( auth('admin')->user() && ( auth('admin')->user()->can('admin.create')))
                <button class="btn btn-primary shadow-md mr-2" type="button" id="addNew">Add New Admin</button>
            @endif
        </div>
        <div id="listContainer" class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            @include('backend.pages.admin.list')
        </div>

    </div>

    @include('backend.partials.script.index')
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#addNew").on('click', () => {
                let url = "{{route('dashboard.admin.create')}}";
                ajaxGet(url, 'GET', {}, showData);
            });
